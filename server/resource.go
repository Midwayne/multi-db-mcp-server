...
// InitializeResources adds the resource definitions to the MCP server. (To be decommissioned since we'll use caching)
func InitializeResources(s *server.MCPServer, connManager *config.DBConnections) {
	// The following creates a resource that provides a landscape view of all connected hosts, their databases and collections.
	s.AddResource(mcp.Resource{
		URI:         "config://db",
		Name:        "Mongo Landscape",
		Description: "A landscape view of all connected MongoDB hosts, their databases and collections.",
	}, server.ResourceHandlerFunc(func(ctx context.Context, req mcp.ReadResourceRequest) ([]mcp.ResourceContents, error) {
		landscape := make(map[string]map[string][]string)
		var mu sync.Mutex
		var wg sync.WaitGroup
		...
		for host, conn := range *connManager {
			...
			go func(host string, conn *models.DBConnection) {
				...
				for _, dbName := range dbNames {
					collections, err := client.Database(dbName).ListCollectionNames(ctx, nil)
					if err != nil {
						// Skip this database if we can't list its collections
						continue
					}
					hostData[dbName] = collections
				}
